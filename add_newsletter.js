...
const css = `
  ...
  .nl-content { position: relative; background: var(--raw-color-velvet-900); border: 1px solid var(--foreground-accent-muted); border-radius: 16px; padding: 3rem 2.5rem; max-width: 440px; width: 90%; transform: translateY(20px); transition: transform 0.4s cubic-bezier(0.16, 1, 0.3, 1); box-shadow: 0 24px 48px rgba(0,0,0,0.5); text-align: center; }
  .nl-modal.is-open .nl-content { transform: translateY(0); }
  .nl-close { position: absolute; top: 1rem; right: 1rem; background: transparent; border: none; color: var(--foreground-accent-muted); cursor: pointer; transition: color 0.2s ease; padding: 0.5rem; display: flex; align-items: center; justify-content: center; }
  .nl-close:hover { color: var(--foreground-accent); }
  .nl-title { font-family: var(--typeface-display); font-size: 2.5rem; color: var(--foreground-accent); margin-bottom: 0.5rem; line-height: 1.1; }
  .nl-desc { font-family: var(--typeface-ui); font-size: 0.95rem; color: var(--foreground-accent-muted); margin-bottom: 2rem; line-height: 1.5; }
  .nl-form { display: flex; flex-direction: column; gap: 1rem; }
  .nl-input { width: 100%; text-align: center; }
  .nl-success { margin-top: 1rem; font-family: var(--typeface-ui); color: var(--foreground-accent); font-size: 0.95rem; display: none; }
`;
...
<div id="newsletter-modal" class="nl-modal" aria-hidden="true">
  ...
  <div class="nl-content">
    ...
    <form id="nl-form" class="nl-form">
      <input type="email" placeholder="Your email address" required class="book-input nl-input" />
      <button type="submit" class="btn-pill btn-fill interactive-btn nl-submit" style="width: 100%;">
...
// Insert HTML before </main> or </body>
...
console.log('Newsletter modal injected.');
