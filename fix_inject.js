...
// 1. We must replace the `#global-bg-video` HTML with the fixed `scene-video` and `scene-canvas`
const videoElements = `
...
if (html.includes('id="global-bg-video"')) {
  html = html.replace(/<video id="global-bg-video"[^>]*><\/video>/, videoElements);
  console.log("Replaced global-bg-video with scene-video and scene-canvas.");
}

// 2. Mobile CSS replacement
html = html.replace(/#global-bg-video \{ display: none !important; \}/g, '#scene-video, #scene-canvas { display: none !important; }');
html = html.replace(/#global-bg-video/g, '#scene-video');

// 3. Inject script right before </body>
const scriptBlock = `
<script>
(function() {
  ...
  function rAF(now) {
    ...
    if (dur > 0 && !reverted) {
      ...
      if (ready && bank.length > 0) {
        ...
        if (video && Math.abs(video.currentTime - current) > 0.01) {
  ...
  async function drawFrame() {
    if (drawBusy || bank.length === 0 || !canvas) return;
    drawBusy = true;
...
if (!html.includes('id="scene-canvas" width="1920"')) {
    html = html.replace('</body>', scriptBlock + '\n</body>');
    console.log("Injected script block.");
} else {
    // If we mistakenly appended it before, replace it
    console.log("Canvas already present somewhere.");
}
